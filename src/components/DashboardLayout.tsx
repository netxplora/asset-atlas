    ...
    <>
      <div className={`p-4 border-b border-sidebar-border ${collapsed ? 'flex justify-center' : ''}`}>
        <Link to="/" className={`flex items-center gap-3 font-bold text-xl ${collapsed ? 'justify-center' : ''}`} onClick={onNavigate}>
          ...
          {!collapsed && <span className="font-heading tracking-tight text-sidebar-foreground">AssetVault</span>}
        </Link>
      </div>
      <nav className="flex-1 py-4 overflow-y-auto overflow-x-hidden px-3 space-y-6">
        {navGroups.map((group, groupIdx) => (
          <div key={groupIdx} className="space-y-0.5">
            {!collapsed && (
              <div className="px-3 text-[10px] font-bold text-sidebar-foreground/40 uppercase tracking-widest mb-2">
                {group.title}
              </div>
            )}
            {group.items.map((item) => {
              ...
              return (
                <Link key={item.to} to={item.to} onClick={onNavigate} title={collapsed ? item.label : undefined}
                  ...
                  <item.icon className={`shrink-0 transition-colors duration-200 ${collapsed ? 'h-5 w-5' : 'h-4 w-4'} ${active ? 'text-primary-foreground' : 'text-sidebar-foreground/50 group-hover:text-sidebar-foreground'}`} />
...
export default function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen flex bg-muted/30">
      ...
      <aside className={`hidden lg:flex flex-col border-r bg-sidebar text-sidebar-foreground transition-all duration-300 ease-in-out relative shadow-sm ${collapsed ? 'w-[68px]' : 'w-64'}`}>
      ...
      <div className="flex-1 flex flex-col min-h-screen min-w-0">
        ...
        <main id="main-content" className="flex-1 p-4 lg:p-8 overflow-auto bg-muted/20 page-enter"><Outlet /></main>
        <ChatWidget />
      </div>
    </div>
  );
